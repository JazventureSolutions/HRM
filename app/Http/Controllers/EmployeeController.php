...
class EmployeeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($logged_user->can('view-details-employee')) {
            ...
            if (request()->ajax()) {
                if ($request->company_id && $request->department_id && $request->designation_id) {
                    ...
                    $employees = Employee::with('user:id,profile_photo,username', 'company:id,company_name', 'department:id,department_name', 'designation:id,designation_name', 'officeShift:id,shift_name')
                        ...
                        ->where('is_active', 1)
                        ->when($request->employee_status, function ($query, $request) {
							return $query->whereNull('exit_date')->orWhere('exit_date', '0000-00-00');
						}, function ($query) {
                ...
                return datatables()->of($employees)
                    ...
                    ->addColumn('name', function ($row) {
						...
						$card_no = "<span>" . __('Card Number') . ": &nbsp;" . ($row->card_no ?? '') . "</span>";
                        ...
                        $payslip_type = "<span>" . __('file.Payslip Type') . ": &nbsp;" . ($row->payslip_type ?? '') . "</span>";

                        return "<div class='d-flex'>
                                        <div class='mr-2'>" . $profile_photo . "</div>
                                        <div>"
                            . $name . '</br>' . $username . '</br>'. $card_no . '</br>' . $gender . '</br>' . $salary . '</br>' . $payslip_type;
                        "</div>
                                    </div>";
                    })
    ...
    public function infoUpdate(Request $request, $employee)
    {
        ...
        if ($logged_user->can('modify-details-employee')) {
            if (request()->ajax()) {
                ...
                $data['designation_id'] = $request->designation_id;
                $data['office_shift_id'] = $request->office_shift_id;
                $data['status_id'] = !empty($request->status_id) ? $request->status_id : null;
                ...
                try
                {
                    ...
                    Employee::whereId((int) $employee)->update($data);
