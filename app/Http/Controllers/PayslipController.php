...
class PayslipController extends Controller {
			...
			if (request()->ajax())
			{
				...
				return datatables()->of($payslips)
					...
					->addColumn('salary_details', function ($row)
					{
                        ...
                        $data .= "<div class='d-flex'>
									<div class='ml-auto'>(+) Special Allowance:</div>
									<div class='ml-auto'>".$row->special_allowance."</div>
								</div>";
                        ...
                        $data .= "<div class='d-flex'>
										<div class='ml-auto'>(-)Advance Deduction &nbsp:</div>
										<div class='ml-auto'>".$row->advance_deduct."</div>
									</div>";
                        $data .= "<div class='d-flex'>
										<div class='ml-auto'>(-)Punishment &nbsp:</div>
										<div class='ml-auto'>".$row->punishment."</div>
									</div>";
						//Net Payable
						$data .= "<div class='d-flex'>
	...
	public function show(Payslip $payslip)
	{
        ...
        $month = date('m', strtotime($payslip->month_year));
        $year = date('Y', strtotime($payslip->month_year));


        $overtime_sum = Attendance::where('employee_id',$payslip->employee_id)
			->whereYear('attendance_date',$year)
			->whereMonth('attendance_date',$month)
			->sum(DB::raw("TIME_TO_SEC(overtime)"));
        $total_overtime = 0;
        $total_overtime_hours = gmdate("H:i:s", $overtime_sum);

        sscanf($total_overtime_hours, '%d:%d', $overtimeHour, $overtimeMin);
        $total_overtime += $overtimeHour * 60 + $overtimeMin;
        $overtime_amount = ($payslip->basic_salary / 60) * $total_overtime * 1.5;

		return view('salary.payslip.show',compact('payslip','employee','total_hours','amount_hours','overtime_amount'));
	}
