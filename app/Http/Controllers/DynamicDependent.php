...
class DynamicDependent extends Controller {
	...
	public function fetchEmployee(Request $request)
	{
		...
		$data = Employee::whereCompany_id($value)->where('is_active',1)->where('exit_date',NULL)->get();
		$output = '';
		foreach ($data as $row)
		{
			$output .= '<option value=' . $row->id . '>' . $row->$first_name . ' ' . $row->$last_name . ' - '. $row->card_no .'</option>';
		}
	...
	public function fetchEmployeeDepartment(Request $request)
	{
		...
		$first_name = $request->get('first_name');
		$last_name = $request->get('last_name');
		$data = Employee::wheredepartment_id($value)->where('is_active',1)->where('exit_date',NULL)->get();
		$output = '';
		foreach ($data as $row)
		{
			$output .= '<option value=' . $row->id . '>' . $row->$first_name . ' ' . $row->$last_name . ' - '. $row->card_no .'</option>';
