                                                    ...
                                                    <div class="table-responsive">
                                                        <table class="table table-bordered mb-0">
                                                            ...
                                                            <tr>
                                                                ...
                                                                <th>Rate * <span class="overtimeAmount" ></span></th>
                                                                <th>Total</th>
                                                            </tr>
                                                            </thead>
                                                                <th>#</th>
                                                                <th id="totalOvertimeDetails"></th>
                                                                <th class="overtimeAmount"></th>
                                                                <th id="totalOvertimeAmount"></th>
                                                            <tbody>
                                                            </tbody>
                                                            {{-- <tr>
                                                                <td><strong>Per Hour Salary:</strong><span class="pull-right" id="basic_salary_amount"></span></td>
                                                                <td class="hide-div"><strong><span id="hours_worked"></span>:</strong> <span class="pull-right" id="total_hours_worked"></span></td>
                                                                <td class="hide-div"><strong><span id="hours_worked_amount"></span>:</strong> <span class="pull-right" id="total_hours_worked_amount"></span></td>
                                                            </tr> --}}
                                                        </table>
                    ...
                    <div class="modal-body">
                        ...
                        <form method="get" id="payment_form" class="form-horizontal" >
                               ...
                               <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="name">{{__('Advance Deduct')}}</label>
                                            <input type="text" name="advance_deduct" id="advanceDeduct" class="form-control" value="0">
                                        </div>
                                    </div>

                                   <div class="col-md-6 hide-element">
                                       <div class="form-group">
                                           <label for="worked_hours">{{__('Total Hours(With Overtime)')}}</label>
                                   ...
                                   <div class="col-md-6 hide-element">
                                       <div class="form-group">
                                           <label for="worked_amount">{{__('Total Amount')}}</label> <a href="#" data-toggle="popover" data-placement="top" data-content="If you don't set this month's amount, the last month's amount will be treated as this month"><i class="fa fa-exclamation-circle text-warning" aria-hidden="true"></i></a>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="name">{{__('Special Allowance')}}</label>
                                        <input type="text" name="special_allowance" id="special_allowance_payment" class="form-control" value="0" >
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="name">{{__('Change Over Time')}}</label>
                                        <input type="number" name="change_over_time" id="change_over_time" class="form-control" value="0" >
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="name">{{__('Punishment')}}</label>
                                        <input type="text" name="punishment" id="punishment_payment" class="form-control" value="0" >
                                    </div>
                                </div>
...
<script type="text/javascript">
    (function($) {
        ...
        $(document).on('click', '.details', function () {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    $('#totalOvertimeDetails').html(result.data.totalOvertimeDetails);
                    $('.overtimeAmount').html(result.data.overtimeAmount);
                    $('#totalOvertimeAmount').html(result.data.totalOvertimeAmount);
                    ...
                    //     - total_deduction + total_other_payment;
                    let total_salary = result.data.totalMonthAmount;
                    ...
                    if (result.data.payslip_type == 'Monthly') {
                        ...
                        $('#hours_worked').html('{{__('Total Hours without overtime')}}');
                        $('#total_hours_worked').html(result.data.monthly_worked_hours);
                        $('#hours_worked_amount').html('{{__('Amount')}}');
        ...
        var pay_total_salary;
        var overTimeRate;
        var totalOverTimeAmount;
        var totalSalary;
        var basicSalary;
        ...
        $(document).on('click', '.generate_payment', function (event) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    pay_total_salary = result.data.total_salary;
                    $('#pension_amount_payment').val(result.data.pension_amount);
                    
                    $('#employee_id').val(result.data.employee);
                    
                    overTimeRate = result.data.overtime_rate;
                    totalOverTimeAmount = result.data.total_overtime;
                    totalSalary = result.data.total_salary;
                    basicSalary = result.data.basic_salary;
        ...
        $('#advanceDeduct').on('keyup', function() {
            if ($(this).val() < 0) {
                $(this).val(0);
            }
            else {
                var final_pay = pay_total_salary + parseFloat($('#special_allowance_payment').val() - $(this).val() - $('#punishment_payment').val());
                $('#net_salary_payment').val(final_pay);
                $('#total_salary_payment').val(final_pay);
            }
        });

        $('#punishment_payment').on('keyup', function() {
            if ($(this).val() < 0) {
                $(this).val(0);
            }
            else {
                var final_pay = pay_total_salary + parseFloat($('#special_allowance_payment').val() - $(this).val() - $('#advanceDeduct').val());
                $('#net_salary_payment').val(final_pay);
                $('#total_salary_payment').val(final_pay);
            }
        });

        $('#special_allowance_payment').on('keyup', function() {
            if ($(this).val() < 0) {
                $(this).val(0);
            }
            else {
                var final_pay = pay_total_salary + parseFloat($(this).val() - $('#advanceDeduct').val() - $('#punishment_payment').val());
                $('#net_salary_payment').val(final_pay);
                $('#total_salary_payment').val(final_pay);
            }
        });
        ...
        $(document).on('change', '#change_over_time', function () {

            if($(this).val() == 0 || $(this).val() == ""){

                $('#total_overtime_payment').val(totalOverTimeAmount);
                $('#net_salary_payment').val(totalSalary);
                $('#total_salary_payment').val(totalSalary);

            }else{

                let tempOverTimeAmount = (overTimeRate * $(this).val()) * basicSalary;
                let tempTotalSalary = totalSalary - totalOverTimeAmount;
                
                tempTotalSalary = tempOverTimeAmount + tempTotalSalary;

                $('#total_overtime_payment').val(tempOverTimeAmount);
                $('#net_salary_payment').val(tempTotalSalary);
                $('#total_salary_payment').val(tempTotalSalary);
            }
        });
