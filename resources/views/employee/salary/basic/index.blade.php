            ...
            <table id="basic_table" class="table ">
                <thead>
                    <tr>
                        ...
                        @endif
                        @if(config('variable.currency_format')=='suffix')
                            <th>{{__('Over Time Rate')}} ({{config('variable.currency')}})</th>
                        @else
                            <th>({{config('variable.currency')}}) {{__('Over Time Rate')}}</th>
                        @endif
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 form-group">
                                ...
                                <select name="payslip_type" id="payslip_type_edit" required class="selectpicker form-control">
                                    ...
                                    <option checked value="Hourly">{{__('Hourly Payslip')}}</option>
                                </select>
                            </div>
                            <div class="col-md-6 form-group">
                                @if(config('variable.currency_format')=='suffix')
                            ...
                            <div class="col-md-6 form-group">
                                @if(config('variable.currency_format')=='suffix')
                                    <label>{{__('Over Time Rate')}} ({{config('variable.currency')}}) *</label>
                                @else
                                    <label>({{config('variable.currency')}}) {{__('Over Time Rate')}} *</label>
                                @endif

                                <input type="text" name="overtime_rate" id="overtime_rate_edit" placeholder="{{__('0.00')}}"  class="form-control">
                            </div>
